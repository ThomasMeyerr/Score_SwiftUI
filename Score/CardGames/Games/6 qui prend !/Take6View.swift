...
struct Take6View: View {
    ...
    func endRound() {
        ...
        if possibleLooser! >= Int(maxScore) {
            ...
            UserDefaults.standard.set(false, forKey: "partyTake6Ongoing")
        } else {
            roundNumber += 1
        }
    }
    ...
    func saveData() {
        UserDefaults.standard.set(true, forKey: "partyTake6Ongoing")
        ...
        if let encodedGameData = try? JSONEncoder().encode(data) {
            UserDefaults.standard.set(encodedGameData, forKey: "Take6GameData")
        }
    }
    
    func loadData() {
        if let data = UserDefaults.standard.data(forKey: "Take6GameData") {
...
#Preview {
    Take6View(numberOfPlayer: 2, maxScore: 66, names: ["Thomas", "Zoé"], isNewGame: false)
        .environment(Data())
}
