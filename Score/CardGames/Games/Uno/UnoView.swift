...
//  Score
//
//  Created by Thomas Meyer on 04/10/2024.
//
...
struct UnoView: View {
    ...
    func saveData() {
        ...
        if let unoHistory = UserDefaults.standard.data(forKey: "UnoHistory") {
            if var decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: unoHistory) {
    ...
    func loadData() {
        if let unoHistory = UserDefaults.standard.data(forKey: "UnoHistory") {
            if let decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: unoHistory) {
                // Check if a same CardGameData exists with the same id
...
#Preview {
    UnoView(id: UUID(), numberOfPlayer: 2, maxScore: 500, names: ["Thomas", "Zoé"], isNewGame: true)
