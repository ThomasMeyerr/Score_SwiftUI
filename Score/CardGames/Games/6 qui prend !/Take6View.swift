...
struct Take6View: View {
    ...
    var body: some View {
        ...
        .navigationTitle(getText(forKey: "take6", forLanguage: data.languages))
    ...
    func saveData() {
        ...
        if let take6History = UserDefaults.standard.data(forKey: "Take6History") {
            if var decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: take6History) {
    ...
    func loadData() {
        if let take6History = UserDefaults.standard.data(forKey: "Take6History") {
            if let decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: take6History) {
                // Check if a same CardGameData exists with the same id
...
#Preview {
    Take6View(id: UUID(), numberOfPlayer: 2, maxScore: 66, names: ["Thomas", "Zoé"], isNewGame: true)
