...
struct SkyjoView: View {
    ...
    func saveData() {
        ...
        if let skyjoHistory = UserDefaults.standard.data(forKey: "SkyjoHistory") {
            if var decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: skyjoHistory) {
    ...
    func loadData() {
        if let skyjoHistory = UserDefaults.standard.data(forKey: "SkyjoHistory") {
            if let decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: skyjoHistory) {
                // Check if a same CardGameData exists with the same id
...
#Preview {
    SkyjoView(id: UUID(), numberOfPlayer: 2, maxScore: 100, names: ["Thomas", "Zoé"], isNewGame: true)
