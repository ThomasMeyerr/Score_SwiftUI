...
struct BeloteSettingsView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(
            getText(forKey: "settings", forLanguage: data.languages) + "Belote"
        )
    ...
    func loadPlayersList() -> some View {
        Section {
            ...
            if numberOfPlayer > 2 {
                Section(
                    "\(getText(forKey: "team", forLanguage: data.languages)) 1"
                ) {
            ...
            if numberOfPlayer > 2 {
                Section(
                    "\(getText(forKey: "team", forLanguage: data.languages)) 2"
                ) {
    ...
    func loadHistory() {
        if let beloteHistory = UserDefaults.standard.data(
            forKey: "BeloteHistory"
        ),
