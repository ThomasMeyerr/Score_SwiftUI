...
struct UnoSettingsView: View {
                        ...
                        if !history.isEmpty {
                            List {
                                ForEach(history) { game in
                                    NavigationLink {
                                        UnoView(id: game.id, numberOfPlayer: game.numberOfPlayer, maxScore: game.maxScore, names: game.names, winner: game.winner)
                                    } label: {
                                        HStack {
    ...
    func loadButtons() -> some View {
        HStack {
            ...
            NavigationLink(getText(forKey: "launch", forLanguage: data.languages)) {
                UnoView(id: nil, numberOfPlayer: numberOfPlayer, maxScore: maxScore, names: names, isNewGame: true)
            }
            .padding()
    ...
    func loadHistory() {
        if let unoHistory = UserDefaults.standard.data(forKey: "UnoHistory"), let decodedHistory = try? JSONDecoder().decode(GameCardHistory.self, from: unoHistory) {
